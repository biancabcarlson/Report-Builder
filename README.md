...
Generate a report using prefilled account information. Review the data, add any missing details, and finalize a consistently formatted report.
...
**Copy Markdown** copies the same report structure used by the Python CLI,
which makes the handoff to a case system or interview notes one click instead
of a manual reformat.
...
The CLI renders multiline values such as `documents_collected` as Markdown
bullets, matching the browser preview.
